...
window.App = {
    ...
    async init() {
        ...
        // Listen for page being restored from bfcache
        window.addEventListener('pageshow', (event) => {
            if (event.persisted) {
                // Page restored from cache – force UI re‑sync
                this.reactivateAfterCache();
            }
        });

        // Also listen for visibility changes (tab switch)
        document.addEventListener('visibilitychange', () => {
            if (document.visibilityState === 'visible' && this.state.currentUser) {
                this.reactivateAfterCache();
            }
        });
    ...
    // Called when the page becomes visible after being backgrounded/cached
    reactivateAfterCache() {
        if (!this.state.currentUser || !this.state.activeRoom) return;

        // Ensure the active chat container is visible and the placeholder is hidden
        $('#chat-placeholder').classList.add('hidden');
        const activeChat = $('#active-chat');
        activeChat.classList.remove('hidden');

        // Force the input area to be interactable
        const input = $('#msg-input');
        if (input) {
            input.disabled = false;
            input.readOnly = false;
            input.placeholder = input.placeholder || 'پیامی بنویسید...';
            input.style.pointerEvents = 'auto';
            this.adjustInputHeight();
            this.checkInputDirection();
        }

        // Force the chat input area to be visible
        ...
        // Refresh room header and user list to reflect any changes
        this.updateRoomHeader();
        this.fetchRoomUsers();
        this.fetchMessages(true);

        // Re‑apply the owner badge
        if (this.state.activeRoom.isOwner) {
        ...
        // Ensure the voice UI is hidden if not recording
        if (!this.state.isRecording && !this.state.recBlob && !$('#voice-ui').classList.contains('hidden')) {
            this.cancelRecord();
        }

        // Re‑start polling if needed
    ...
    async autoJoinLastRoom() {
        ...
        if (!password) {
            ...
            if (password) {
                this.state.memoryKeys[lastRoomName] = password;
            }
        }
    ...
    async confirmLogout() {
        if ($('#chk-logout-nuke').checked) {
            if (confirm("آیا مطمئن هستید؟ این عمل غیرقابل بازگشت است و تمام پیام‌های شما در همه اتاق‌ها حذف خواهد شد.")) {
                ...
                await window.API.post('delete_user_history', {});
                this.performLogout();
            }
        } else {
            this.performLogout();
